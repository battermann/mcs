...
object Games {
  ...
  val game1: (Game, Option[Result[Position, Int]]) = (SameGame.evaluateGameState(Board(board15x15), 0), None)

  def board(size: Int): (Game, Option[Result[Position, Int]]) =
    (SameGame.evaluateGameState(Board(board15x15.take(size).map(col => Column(col.cells.take(size)))), 0), None)
